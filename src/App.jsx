      ...
      <div className="max-w-2xl mx-auto space-y-6 md:space-y-8">
        ...
        {cooldownRemaining > 0 && (
          <div className="bg-orange-500/10 border border-orange-500/50 text-orange-400 px-4 py-3 rounded-xl text-center font-bold text-lg animate-pulse shadow-[0_0_15px_rgba(249,115,22,0.2)]">
            ⏳ Hai già votato! Prossimo voto tra:{" "}
            <span className="text-white">{formatTime(cooldownRemaining)}</span>
          </div>
        )}
        ...
        {results.length > 0 && (
          <div className="space-y-3">
            <h2 className="text-xl font-bold text-neutral-300 border-b border-neutral-700 pb-2">
              Risultati di ricerca
            </h2>
            {results.map((album) => (
              <div
                key={album.id}
                className="flex items-center gap-3 bg-neutral-800/50 p-3 rounded-xl border border-neutral-700"
              >
                {album.cover ? (
                  <img
                    src={album.cover}
        ...
        <div className="space-y-3 mt-10">
          <h2 className="text-xl font-bold text-orange-400 border-b border-orange-500/30 pb-2">
            🏆 Top Richieste
          </h2>
          ...
          {isLeaderboardLoading ? (
            ...
            <div className="flex flex-col items-center justify-center py-10 space-y-4">
              ...
              <p className="text-neutral-400 animate-pulse text-sm">
                Recuperando la classifica...
              </p>
            </div>
          ) : leaderboard.length === 0 ? (
            /* STATO 2: Caricamento finito array vuoto */
            <p className="text-center text-neutral-500 mt-6 italic">
              Nessun album in classifica. Sii il primo a votarne uno!
            </p>
          ) : (
            leaderboard.map((album, index) => (
              <div
                key={album.id}
                className="flex items-center gap-3 bg-neutral-800 p-3 rounded-xl border border-orange-500/20 shadow-md transition-all hover:scale-[1.01]"
              >
                <span className="text-2xl font-black text-neutral-600 w-6 text-center">
                  {index + 1}
                </span>
                {album.cover ? (
                  <img
